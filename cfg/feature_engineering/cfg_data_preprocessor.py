...
def clean_opcode_sequence(opcode_sequence: List[str]) -> Optional[str]:
    ...
    # 过滤掉空字符串和None值，并只保留VALID_OPCODES中的操作码
    cleaned_opcodes = [op.strip().lower() for op in opcode_sequence if op and isinstance(op, str) and op.strip() and op.strip().lower() in VALID_OPCODES]
...
def build_node_mapping(cfg_data: Dict) -> Dict[str, int]:
    ...
    返回：
        从global_id到索引的映射字典
    """
    # 修复：正确访问cfg_data中的global_basic_blocks
    cfg_section = cfg_data.get('cfg_data', {})
    global_nodes = cfg_section.get('global_basic_blocks', [])
...
def construct_edge_index(cfg_data: Dict, node_mapping: Dict[str, int]) -> torch.Tensor:
    ...
    """
    # 修复：正确访问cfg_data中的global_edges
    cfg_section = cfg_data.get('cfg_data', {})
    global_edges = cfg_section.get('global_edges', [])
...
def process_and_save_one_file(cfg_file_path: Path, fasttext_model_path: str, output_dir: str) -> bool:
    """
    加载、处理单个CFG JSON文件，并保存为.pt文件
    
    Args:
        cfg_file_path (Path): 输入的CFG JSON文件路径
    ...
    logger.info(f"[PID {os.getpid()}] 开始处理文件: {cfg_file_path.name}")
    
    try:
        # 调用已有的处理函数
        success = process_single_cfg(str(cfg_file_path), output_dir, fasttext_model)
        
        if success:
            logger.info(f"[PID {os.getpid()}] 成功处理: {cfg_file_path.name}")
            return True
        else:
            logger.warning(f"[PID {os.getpid()}] 处理失败: {cfg_file_path.name}")
            return False
            
    except Exception as e:
        logger.error(f"[PID {os.getpid()}] 处理 {cfg_file_path.name} 时出错: {e}", exc_info=True)
        return False
...
def batch_process_cfg(input_dir: str, output_dir: str, fasttext_model_path: str, num_workers: int = None) -> Dict[str, int]:
    """
    批量处理CFG JSON文件（多进程版本）。
    ...
    logger.info(f"找到 {len(cfg_files)} 个JSON文件需要处理")
    
    # 设置工作进程数量
    if num_workers is None:
        num_workers = os.cpu_count()
    
    # 确保工作进程数量不超过文件数量或CPU核心数
    num_workers = min(num_workers, len(cfg_files), os.cpu_count())
    logger.info(f"使用 {num_workers} 个工作进程进行处理")
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=num_workers) as executor:
        # 提交所有任务
        future_to_file = {
            executor.submit(process_and_save_one_file, cfg_file, fasttext_model_path, output_dir): cfg_file
            ...
            try:
                ...
                if success:
                    ...
                    if successful_count % 100 == 0:
                        logger.info(f"已成功处理 {successful_count} 个文件...")
                else:
                    failed_count += 1
                    logger.warning(f"处理失败: {cfg_file.name}")
                    
            except Exception as e:
                failed_count += 1
                logger.error(f"处理 {cfg_file.name} 时出现异常: {e}")
    
    logger.info(f"多进程批量处理完成: {successful_count} 成功, {failed_count} 失败")
    ...
    try:
        ...
        for json_file in json_files:
            try:
                ...
                # 统计节点和边数量
                cfg_section = cfg_data.get('cfg_data', {})
                nodes = cfg_section.get('global_basic_blocks', [])
                edges = cfg_section.get('global_edges', [])
...
def main():
    ...
    # 设置固定的工作进程数量
    num_workers = 4
    logger.info(f"使用固定的工作进程数量: {num_workers}")
    
    # 运行批量处理
    stats = batch_process_cfg(input_dir, output_dir, fasttext_model_path, num_workers)
